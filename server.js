...
io.on('connection', (socket) => {
  ...
  socket.on(
    ...
    ({ roomId, nickname }) => {
      ...
      io.to(roomId).emit(
        'systemMessage',
        {
          text: `${nickname} telah masuk ke room.`
        }
      );
  ...
  socket.on('disconnect', () => {
    ...
    io.to(roomId).emit(
      'systemMessage',
      {
        text: `${socket.nickname} telah keluar dari room.`
      }
    );
...
server.listen(PORT, () => {
  console.log(
    'Indonesia Chat Go Server running'
  );
});});
...
server.listen(PORT, () => {
  console.log('Indonesia Chat Go Server running');
});
